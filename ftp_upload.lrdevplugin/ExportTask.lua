...
function ExportTask.processRenderedPhotos( functionContext, exportContext )
	...
	Utils.message("destParentPath : " .. destParentPath)
	for _, rendition in exportContext:renditions{ stopIfCanceled = true } do
		local success, pathOrMessage = rendition:waitForRender()
		if progressScope:isCanceled() then break end
		if success then
			local filename = LrPathUtils.leafName( pathOrMessage )
			Utils.message("in render loop filename: " .. filename .. " path: " .. pathOrMessage)
			if not success then
				table.insert( failures, filename )
			end
			local dest = LrPathUtils.child(destParentPath, filename)
			if LrFileUtils.exists(dest) then 
				Utils.message("dest file " .. dest .. " already exists. Overwriting")
			end
			LrFileUtils.copy(pathOrMessage, dest)
			Utils.message("copy " .. pathOrMessage .. "  -->  " .. dest )
